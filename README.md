...
**"这周赚钱了所以规则对"是错的** —— 运气、市场环境、样本太少,
任何一个都能造出那个结论。所以一条规则要改变状态(从而改变它能调动多少真钱),
必须**同时**过四关:

| | 问题 | 不过会怎样 |
|---|---|---|
| ① | 样本够不够多(≥ 10 谈方向,≥ 20 谈改状态) | 「数据不足」 |
| ② | 前后两段结论一致吗 | 「环境可能变了」,**禁止改状态** |
| ③ | 差异超过组内波动吗(信噪比 ≥ 1.0 / ≥ 2.0) | 「差异在噪音范围内」 |
| ④ | 最近 2 次审计同向吗 | 「判定在翻转,翻转期不要改状态」 |

**② 是关键的一关。** 一条只在下跌趋势里成立的规则,在市场转多后必然连续"失效"。
只有「支持 / 不支持」两个出口时,它会被判成**规则错了** ——
然后你在市场即将回到它擅长的环境时,亲手把它退役。

所以审计有第三个出口 🔀「环境可能变了」:前后两段结论相反就**禁止改状态**,
提示你补一个适用环境的条件、分环境重新计数。
**判"规则错了"的门槛因此高于判"没用":必须前后两段都不支持,一次反转只回观察期。**

四关全过,审计也**只产出建议** —— `--set-status` 不带 `--approved` 直接退出。
**状态变更改变的是这条规则能否指导真钱下单,那是你的决定,不是 AI 的。**
