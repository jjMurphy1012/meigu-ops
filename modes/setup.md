...
> ⚠️ **不要擅自替用户编造市场判断类规则。** 用户没有明确选择路线 2 之前,
> 没有市场规则时系统只能跑分析与 dry-run。这是设计如此,不是缺陷。

### 路线 2:用户选择让 AI 代拟

只有用户**明确说了**"我不想自己定策略"时才走这条。顺序不能变:

**① 先说免责,再谈规则。** 原话要点:

> 我可以根据当下的行情与你的持仓提出几条规则,但要先说清楚三件事:
> 一、我提的策略同样可能是错的,它没有经过你的账户数据检验;
> 二、我不掌握你的风险承受能力、资金用途和时间尺度;
> 三、最终的交易决策与结果由你负责,这不构成投资建议。
> 写进去之后,它和你自己写的规则走完全一样的流程 —— 从最低仓位档起步,
> 靠你的台账数据来检验,数据不支持就会被降级。

**② 逐条把规则原文念给用户**,包括:一句能被证伪的 `statement`、
`kind`(市场判断 / 流程纪律)、以及**打算怎么检验它**(`test`)。
不要一次抛十条 —— 2~3 条,每条都说清"什么情况算它错了"。
...
不加 `--approved` 会被脚本直接拒绝。写入时自动记 `origin = "ai"` 与
`approved_at`(你确认的时间)—— 有了这两个字段才分得清
"用户选择了信任 AI" 和 "AI 自己往文件里写了一条"。

**④ 之后就是普通规则了。** 同样 `hypothesis` 起步、同样最低档尺寸、
同样要靠台账数据才能升级、同样会被推翻。复盘时可以按 `origin` 分开看:
**你自己写的和 AI 代拟的,哪一类被数据推翻得更多?**
